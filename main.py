...
character = Character(
    [
        ...
        "C:/Users/user/OneDrive/바탕 화면/python Workspace/game_images/gray spider(left)_player.png",
        "C:/Users/user/OneDrive/바탕 화면/python Workspace/game_images/gray spider(right)_player.png"
    ],
...
def is_same_tile(rect1, rect2):
    return (rect1.x // tile_size == rect2.x // tile_size) and (rect1.y // tile_size == rect2.y // tile_size)

# 헬퍼 함수들 (위치 초기화 등 기존 코드 유지)
...
def reset_character_for_stage(character, center_pos):
    character.reset(center_pos)
    character.move_dir = pygame.math.Vector2(0, 0)
    character.moving = False
    character.anim_timer = 0
    character.anim_index = 0

def center_player_and_clear_movement(player):
    player.rect.center = (screen_width//2, screen_height//2)
    if hasattr(player, "pos"):
        player.pos.x, player.pos.y = float(player.rect.x), float(player.rect.y)
    if hasattr(player, "move_dir"):
        player.move_dir = pygame.math.Vector2(0, 0)
    player.moving = False
...
while running:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if game_state == 'start':
                ...
                if next_stage_button_rect.collidepoint(mouse_pos):
                    game_state = 'play2'
                    reset_character_for_stage(character, (screen_width//2, screen_height//2))
    ...
    if game_state == 'start':
        ...
        monster_manager.update(dt, character.rect.center, screen.get_rect())
        ...
        for m in monster_manager.monsters:
            if is_same_tile(character.rect, m.rect) and character.hit_timer <= 0:
        ...
        character.update(dt, screen.get_rect())
        character.draw(screen)
        monster_manager2.update(dt, character.rect.center, screen.get_rect())
